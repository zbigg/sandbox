...
//
// for actual value, retuen resolved promise
// fpr promise, just return it
//
...
int main()
{
    ...
    vec.push_back(Promise<int>().resolve(1));
    vec.push_back(Promise<int>().resolve(2));
    vec.push_back(Promise<int>().resolve(3));
    vec.push_back(Promise<int>().resolve(4));

    std::vector<Promise<int>> vec2 = map<int,int>(vec, [](int v) -> int {
        return 2*v;
    });
    reduce(vec2, 0, [](int v, int acc) -> int {
        return acc+v;
    }).then([](int result) {
